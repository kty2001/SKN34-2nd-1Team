...
4명의 분석 결과를 종합한 결과, 헬스장 고객 이탈은 **가입 초기, 짧은 계약 기간, 낮은 방문 빈도**와 관련성이 높은 것으로 확인되었다.

특히 분석 결과에서 **이용 기간(Lifetime), 계약 기간, 방문 빈도**가 반복적으로 주요 이탈 요인으로 나타났으며, 가입 기간이 짧고 방문 빈도가 낮은 고객일수록 이탈 위험이 높게 나타나는 공통적인 패턴을 확인하였다.

또한 가입 초기 고객의 이탈이 집중적으로 발생했으며, 계약 기간이 길어질수록 이탈 위험이 감소하는 경향도 확인되었다.

모델 측면에서는 분석별로 **K-Means, Random Forest, XGBoost, 생존분석** 등 다양한 방법을 적용하여 고객 이탈을 여러 관점에서 분석하였다. 그 결과 단순히 이탈 여부를 예측하는 것뿐만 아니라 **고객의 이탈 위험도와 이탈 특성을 파악하고 고객군별 특성을 분석할 수 있었다.**

따라서 이번 분석의 핵심 결과는 **고객의 이용 기간과 계약 기간, 실제 방문 행동이 이탈 위험을 판단하는 주요 요인**이라는 점이며, 특히 **가입 초기 및 이용 빈도가 낮은 고객에 대한 관리가 중요하다**는 것이다.

이를 바탕으로 향후에는 분석 결과를 활용하여 고객별 이탈 위험을 사전에 파악하고, 고객 특성에 맞는 **맞춤형 고객 유지 전략을 수립할 필요가 있다.**
